...
export default function ModalAdd(props){
    ...
    return(
        <>
            ...
            <aside className="bg-[#f2f2f2] dark:bg-black w-3/5 h-1/2 h-min-300px fixed top-1/2 left-1/2 -translate-1/2 z-10 flex flex-col justify-between md:flex-row">
                ...
                <div className="flex flex-col gap-2 py-6 px-3 w-full h-2/3 relat md:w-1/2 md:h-full md:p-6 md:gap-7 ">
                    <h2 className="text-4xl">{t('modalImage.image')}</h2>


                    <div className="w-full">
                        <input type="text" maxLength="20" required placeholder={t('modalImage.titleInput')} className="border-b p-2 w-full" value={title} onChange={(e) => setTitle(e.target.value)}/>
                        <input type="text" placeholder={t('modalImage.urlLink')} required className="border-b p-2  w-full" value={urlImg} onChange={(e) => setUrlImg(e.target.value)}/>
                    </div>


                    <div className="flex gap-2">
                        <input type="checkbox" name="favorite" id="favorite" checked={favorite} onChange={() => setFavorite(!favorite)} className="accent-black "/>
                        <label htmlFor="favorite">{t('modalImage.favorite')}</label>
                    </div>
                ...
                <figure className="h-1/3 md:h-full md:w-1/2 p-10">
                    {urlImg && (
                        <>
                            ...
                            : imgValid === false ? (
                                    <p className="w-full h-full flex justify-center items-center text-red-500">{t('modalImage.error')}</p>
                                ) 
                            : 
                                <p>{t('modalImage.validating')}</p>}
